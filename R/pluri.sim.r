...
all_results_categorical_variable <- matrix(NA, nrow = m1, ncol = (nrealiz / nvar))
all_results_coord <- matrix(NA, nrow = m1, ncol = ncol(simucoord))
for (n in 1:nloops){
...
# =============================================
all_results_coord[index, ] <- coord
i_seq <- seq(1,nrealiz,nvar)
k <- 0
for(j in 1:(length(i_seq))){
k <- k + 1
y_simu <- simu[,i_seq[j]:(i_seq[j]+1)]
all_results_categorical_variable[index, k] <- t(t(pluri_truncate(y_simu, nfield, flag, nthres, thresholds)))
}
}
colnames(all_results_coord) <- c("X", "Y", "Z")
plurigaussian_results <- list(coord = all_results_coord, categoricalVar = all_results_categorical_variable)
}
